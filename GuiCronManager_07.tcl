...
# Максимизируем окно (кроссплатформенный способ)
#catch {wm attributes . -zoomed 1}
#catch {wm state . zoomed}
#catch {wm geometry . [winfo screenwidth .]x[winfo screenheight .]}
...
# 6. Список задач с информацией в строке (как в Python версии)
labelframe .main.list -text "Активные задачи" -padx 5 -pady 5

# Создаем список с информацией о задачах
...
proc browseScript {} {
    ...
    if {$filename != ""} {
        ...
        # Автоматически заполнить название из имени файла, если не задано
        if {$taskName == ""} {
...
proc parseHumanTime {timeStr} {
    ...
    if {[regexp {(\d{1,2})\.(\d{1,2})\.(\d{4})\s+(\d{1,2}):(\d{2})} $timeStr -> day month year hour minute]} {
        ...
        # Для прошлых дат возвращаем специальное выражение
        if {$year < $currentYear} {
    ...
    # Дни недели с правильными формами (м.р./ж.р.)
    array set weekdays {
    ...
    foreach {day_name day_num} [array get weekdays] {
        # Проверка шаблона "каждый понедельник в XX:XX"
        ...
        # Проверка шаблона "каждую среду в XX:XX" для дней женского рода
        ...
        # Простой формат дня недели: "суббота 9:30"
...
proc addTask {} {
    ...
    # Преобразование формата времени в cron
    ...
    if {[catch {installCronJob $task} errorMsg]} {
        tk_messageBox -title "Ошибка" -icon error -message "Не удалось добавить задачу: $errorMsg"
        return
    }
    ...
    set taskName ""
    set timeFormat ""
    # Оставляем галочку GUI приложения включенной
    
    log "Задача '$taskName' добавлена успешно"
}

# Процедура для установки задачи в системный cron
proc installCronJob {task} {
    global execLogDir
    array set taskArray $task
    
    # Логирование для отладки
    log "Установка задачи: $taskArray(name) с cron-выражением: $taskArray(cron)"
    
    # Получить текущий crontab с корректной обработкой ошибок
    if {[catch {set current_cron [exec crontab -l]} result]} {
        log "Текущий crontab не найден, создаем новый"
        set current_cron ""
    }
    
    # Убедимся, что в конце crontab есть перевод строки
    if {$current_cron != "" && ![string match "*\n" $current_cron]} {
        append current_cron "\n"
    }
    
    # Проверить, существует ли уже эта задача
    set comment "# CronManager: $taskArray(name)"
    if {[string first $comment $current_cron] != -1} {
        log "Задача '$taskArray(name)' уже существует в cron, обновляем"
        removeFromCron $task
        
        # Получить обновленный crontab
        if {[catch {set current_cron [exec crontab -l]} result]} {
            set current_cron ""
        }
    }
    
    # Определить интерпретатор по расширению
    ...
    # Лог-файл для задачи
    set logFile [file join $execLogDir "cron_manager_$taskArray(name).log"]
    
    # Для GUI-приложений нужен специальный wrapper
    if {$taskArray(is_gui)} {
        ...
        # Создать wrapper-скрипт для GUI
        set wrapperScript [file join $execLogDir "cron_manager_$taskArray(name)_wrapper.sh"]
        
        set f [open $wrapperScript w]
        puts $f "#!/bin/bash"
        puts $f "# Скрипт-обертка для запуска GUI приложения из cron"
        puts $f "# Создан автоматически CronManager"
        puts $f ""
        puts $f "# Запись в лог"
        puts $f "echo \"\[\$(date)\] Запуск $taskArray(script) через wrapper\" >> $logFile"
        puts $f ""
        puts $f "# Установка переменных окружения для GUI"
        ...
        puts $f "echo \"\$(date '+%Y-%m-%d %H:%M:%S')\" > \"$execLogDir/cron_manager_$taskArray(name)_lastrun.txt\""
        puts $f ""
        puts $f "# Запуск GUI приложения"
        ...
        # Использовать wrapper для задачи
        set cronLine "$taskArray(cron) $wrapperScript"
        log "Создан wrapper для GUI: $wrapperScript"
    } else {
        # Обычная задача (не GUI)
        # Создать wrapper-скрипт для не-GUI приложений (для логирования времени запуска)
        set wrapperScript [file join $execLogDir "cron_manager_$taskArray(name)_wrapper.sh"]
        
        set f [open $wrapperScript w]
        puts $f "#!/bin/bash"
        puts $f "# Скрипт-обертка для запуска скрипта из cron"
        puts $f "# Создан автоматически CronManager"
        puts $f ""
        puts $f "# Запись в лог"
        puts $f "echo \"\[\$(date)\] Запуск $taskArray(script) через wrapper\" >> $logFile"
        ...
        # Использовать wrapper для задачи
        set cronLine "$taskArray(cron) $wrapperScript"
        log "Создан wrapper для обычного скрипта: $wrapperScript"
    }
    
    # Добавить задачу в crontab
    set new_cron "$current_cron\n$comment\n$cronLine\n"
    ...
    # Установить новый crontab с корректной обработкой ошибок
    if {[catch {exec crontab $tempFile} errorMsg]} {
        log "ОШИБКА установки crontab: $errorMsg"
        file delete $tempFile
        error "Не удалось установить crontab: $errorMsg"
    }
    ...
    set taskType "GUI"
    if {!$taskArray(is_gui)} {
        set taskType "Обычная"
    }
    log "Задача установлена в cron ($taskType): $cronLine"
    
    # Установить права на выполнение для скрипта
    if {[catch {exec chmod +x $taskArray(script)} errorMsg]} {
        log "Предупреждение: не удалось установить права на выполнение: $errorMsg"
    } else {
        log "Установлены права на выполнение для $taskArray(script)"
    }
...
proc verifyCronInstallation {task} {
    ...
    set comment "# CronManager: $taskArray(name)"
    if {[string first $comment $current_cron] != -1} {
        log "Проверка: задача '$taskArray(name)' найдена в crontab"
        return 1
    } else {
        log "Предупреждение: задача '$taskArray(name)' НЕ найдена в crontab после установки!"
        return 0
    }
}

# Процедура для удаления задачи из cron
proc removeFromCron {task} {
    ...
    # Поиск и удаление строк с задачей
    ...
    foreach line $lines {
        ...
        if {[string first "# CronManager: $taskArray(name)" $line] != -1} {
            set skip_next 1
            continue
        }
    ...
    # Удалить файл последнего запуска
    set lastRunFile [file join $execLogDir "cron_manager_$taskArray(name)_lastrun.txt"]
...
proc removeTask {} {
    ...
    # Удалить wrapper-скрипт, если он существует
    set wrapperScript [file join $execLogDir "cron_manager_$taskArray(name)_wrapper.sh"]
    if {[file exists $wrapperScript]} {
        if {[catch {file delete $wrapperScript} errorMsg]} {
            log "Не удалось удалить wrapper-скрипт: $errorMsg"
        } else {
...
proc testScript {} {
    ...
    if {[catch {
        ...
        if {$result != ""} {
            # Ограничение вывода до 200 символов
...
proc checkLastRunTimes {} {
    ...
    foreach task $tasks {
        ...
        # Проверить файл последнего запуска
        set lastRunFile [file join $execLogDir "cron_manager_$taskArray(name)_lastrun.txt"]
        if {[file exists $lastRunFile]} {
            if {[catch {
                ...
                # Обновить время последнего запуска в задаче
                array set taskArray [list last_run $lastRunTime]
            } errorMsg]} {
                log "Ошибка чтения времени запуска для $taskArray(name): $errorMsg"
            }
        } else {
            array set taskArray [list last_run "Никогда"]
        }
        
        # Пересобрать задачу с обновленными данными
        set updatedTask {}
...
proc showTaskDetails {index} {
    ...
    if {$index >= 0 && $index < [llength $tasks]} {
        ...
        set logFile [file join $execLogDir "cron_manager_$taskInfo(name).log"]
        if {[file exists $logFile]} {
            append detailMessage "\nЛог-файл: $logFile\n"
            
            # Добавить последние строки лога, если он существует
            if {![catch {
                set f [open $logFile r]
                set logContent [read $f]
                close $f
                
                # Получить последние 5 строк лога
                ...
                # Если не можем прочитать лог, не выводим ошибку
            }
        }
...
proc refreshTaskList {} {
    ...
    # Проверка времен последнего запуска (без обратного вызова refreshTaskList)
    set tasks [checkLastRunTimes]
    
    # Очистить список
    .main.list.tasks delete 0 end
    
    # Заполнить данными с информацией о задаче в строке
    foreach task $tasks {
        array set taskArray $task
        ...
        set lastRun "Никогда"
        if {[info exists taskArray(last_run)]} {
            set lastRun $taskArray(last_run)
        }
        
        # Создаём строку с основной информацией о задаче
        #set displayLine "$taskArray(name) | $scriptName | $taskArray(time_format) | $taskType | $taskArray(status) | Посл.запуск: $lastRun"
...
proc saveTasks {} {
    ...
    if {[catch {
        ...
        puts $f "# Формат: name|script|time_format|cron|is_gui|status|last_run"
        puts $f ""
        
        foreach task $tasks {
            array set taskArray $task
            
            # Преобразование булева значения в 0/1
            set isGui "0"
            if {$taskArray(is_gui)} {
                set isGui "1"
            }
            
            # Значение последнего запуска
            set lastRun "Никогда"
            if {[info exists taskArray(last_run)]} {
                set lastRun $taskArray(last_run)
            }
            
            puts $f "$taskArray(name)|$taskArray(script)|$taskArray(time_format)|$taskArray(cron)|$isGui|$taskArray(status)|$lastRun"
        }
        close $f
    } errorMsg]} {
    ...
    if {[catch {
        ...
        while {[gets $f line] >= 0} {
            ...
            if {[llength $parts] >= 5} {
                ...
                set script [lindex $parts 1]
                set time_format [lindex $parts 2]
                set cron [lindex $parts 3]
                
                # Проверка на наличие флага GUI
                set is_gui 0
                set status "Активна"
                set last_run "Никогда"
                
                if {[llength $parts] >= 6} {
                    set is_gui [expr {[lindex $parts 4] == "1"}]
                    set status [lindex $parts 5]
                    
                    if {[llength $parts] >= 7} {
                        set last_run [lindex $parts 6]
                    }
                } else {
                    set status [lindex $parts 4]
                }
...
proc startLastRunChecker {} {
    ...
    # Проверяем время последнего запуска и обновляем список задач
    set tasks [checkLastRunTimes]
    refreshTaskList
...
proc init {} {
    ...
    # Запустить проверку времени последних запусков
    startLastRunChecker
    
    # Центрирование окна после полной загрузки интерфейса
